...
class DBEngine(object):
    ...
    def _fetch(self, query, size=-1, commit=True):
        query = query.strip()
        # self.logger.info(f'execute sql: {query[:250]+'........' if len(query) > 250 else query}')
        self.logger.info(f'execute sql: {query}')
